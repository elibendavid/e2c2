...
class ec2_controller(cmd.Cmd):
    ...
    def do_list(self, param1="no"):
        ...
        if param1 == "all":
            ...
            if not _instances:
                self.print_warning(
                    "no instances of interest specified in configuration, showing all instances ('list all-running')")
                all_instances = self.ec2.instances.filter(Filters=[
                    {
                        'Name': 'instance-state-name',
                        'Values': ['running']
            ...
            if not _instances:
                self.print_warning(
                    "no instances of interest specified in configuration, showing all instances ('list all')")
                all_instances = self.ec2.instances.all()
            else:
                all_instances = self.ec2.instances.filter(Filters=[
                    {
        ...
        names_upper = [name.upper() for name in self.menu[0].keys()]
        table_data = [names_upper]
    ...
    def validate_configuration(self):
        ...
        if 'instances' not in self.config:
            self.print_warning(
                "configuration does not contain any instance info, you will only be able use 'all' functions (list all, list all-running)")
